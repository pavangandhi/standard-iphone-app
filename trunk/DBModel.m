...
+(sqlite3*) getDatabase {
    return [[[self class] getDBConnection] getDatabase];
}

+(DBConnection*) getDBConnection {
    standard_iphone_appDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
    DBConnection *connection =  [appDelegate getCurrentDBConnection];
    return connection;
}
...
-(void) checkTable {
    ...
    if([[[self class] getDBConnection] didCheckTable:[[self class] tableName]]==TRUE) {
        return;
    } 
    
    // Füge aktuelle Tabelle zu den überprüften Tabellen hinzu
    [[[self class] getDBConnection] addCheckedTable:[[self class] tableName]];
    
    // Wenn Tabelle nicht existiert, dann erstelle eine Tabelle
    if([[[self class] getDBConnection] tableExists:[[self class] tableName]] == FALSE) {
        [self createTable];
        [self createIndizes];
    }
...
+(id)findByPK:(int)inPk {
	NSArray *object = [[self class] findByCriteria:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE pk = %ll LIMIT 1", [[self class] tableName], inPk]];
...
-(BOOL) delete {
	...
	NSString *deleteQuery = [NSString stringWithFormat:@"DELETE FROM %@ WHERE pk = %ll", [[self class] tableName], pk];
...
-(BOOL) save {
    ...
    if(result == SQLITE_OK) {
        ...
        for (NSString *propName in props)
        {
            ...
            if ([propType isEqualToString:@"i"] || [propType isEqualToString:@"f"])
            {
